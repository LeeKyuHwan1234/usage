...
Codex와 Claude Code의 구독형 사용량을 Windows 시스템 트레이와 작업표시줄 위 배지에서 확인하는 작은 도구입니다. 트레이 아이콘이나 배지를 우클릭하면 둥근 상세 대시보드가 열립니다.
...
- 작업표시줄 배지: Codex / Claude의 5h·7d 사용률과 각각의 상태 색 점 표시
...
등록 후에는 작업표시줄 오른쪽의 숨겨진 아이콘 영역에서 **AI Usage Dashboard** 아이콘을 우클릭해 사용량을 봅니다.

작업표시줄 오른쪽 위에는 다음 형식의 작은 배지가 표시됩니다.
...
각 색 점은 해당 항목의 사용률을 뜻합니다. 배지는 좌클릭으로 드래그해 위치를 옮길 수 있으며, 옮긴 위치는 다음 실행에도 유지됩니다. 배지 또는 트레이 아이콘을 우클릭하면 상세 대시보드가 열립니다.

> Windows 알림 영역 내부에는 앱이 임의의 텍스트 UI를 직접 삽입할 수 없습니다. 따라서 배지는 알림 영역 바로 위에 표시됩니다.
...
트레이 아이콘 또는 배지를 우클릭해 상세 대시보드를 연 뒤 다음 버튼을 사용합니다.

- `Configure`: Codex / Claude 표시 여부 변경
- `Hide badge` / `Show badge`: 작업표시줄 배지를 숨기거나 다시 표시
- `Exit`: 프로그램 완전 종료

배지 오른쪽 위의 `X`도 배지를 숨깁니다. 숨긴 배지는 트레이 아이콘 우클릭 → `Show badge`로 다시 표시할 수 있습니다.
...
이 폴더 전체를 ZIP으로 압축해 전달하세요. 받는 사람은 압축을 풀고 `AI Usage Dashboard.vbs`를 더블클릭하면 됩니다.
